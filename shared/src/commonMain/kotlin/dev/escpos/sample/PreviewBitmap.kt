 ...
 * [ImageBitmap] for [SampleAppState.previewBitmap]. There is no portable "write a raw ARGB pixel
 * array into an `ImageBitmap`" API in common Compose (Android/Skia back it differently), so this
 * draws instead: white background, then one black [androidx.compose.ui.graphics.drawscope.DrawScope.drawRect]
 * per horizontal run of set bits — the same [CanvasDrawScope] technique
 * [dev.escpos.render.ReceiptRenderer] itself uses to build a bitmap in common code, and far fewer
 * draw calls than one rect per dot for typical text/QR content.
 */
public fun rowsToBitmap(dotWidth: Int, rows: List<ByteArray>): ImageBitmap {
